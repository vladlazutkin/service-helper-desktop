...
import { OBJECT_TAG, TAG, VIEW_HEIGHT, VIEW_WIDTH } from '../constants';
...
const img = createImgSprite(
  require('renderer/assets/images/games/terraria/sprites/PC Computer - Terraria - Eye of Cthulhu & The Twins.png')
);

export class Enemy extends Object implements BaseObject, LiveObject {
  speed: number;
  jumpSpeed: number;
  ...
  constructor(x: number, y: number, hp: number) {
    super(x, y, spriteWidth / 1.5, spriteHeight / 1.5);
    this.totalHP = hp;
    this.HP = hp;
  ...
  copy() {
    return new Enemy(this.x, this.y, this.HP);
  }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    const position = Math.floor(this.gameFrame / 10) % anim.loc.length;
